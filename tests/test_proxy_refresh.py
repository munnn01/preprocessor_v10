...
def test_ordinary_resume_still_rejects_proxy_changes():
    ...
    assert not train.validate_resume_proxy(checkpoint, "new.pt", "old", refresh=False)
    with pytest.raises(ValueError, match="no proxy SHA-256"):
        train.validate_resume_proxy(
            {"args": {"proxy_checkpoint": "old.pt"}},
            "new.pt", "new", refresh=False,
        )
...
def test_kaggle_recipe_restores_saved_options_without_old_initialization():
    from kaggle_cells.v4_swin_rate_recovery import _saved_training_cli
    ...
    cli = _saved_training_cli(saved, overrides, parser)
...
def test_main_refresh_checks_before_epoch8_and_restores_adam_state(tmp_path, monkeypatch, accepted):
    ...
    command = [
        "train.py", "--preprocessor", "swin", "--codec", "h264",
        "--codec-qps", "30", "35", "40", "45",
        "--train-codec-source", "real", "--rate-lambda", "0",
        "--rate-dual-control", "--rate-dual-parity-lambda", "0.05",
