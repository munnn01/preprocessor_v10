...
def test_v4_recovery_recipe_uses_only_proxy_rate_objective(tmp_path, monkeypatch):
    ...
    saved = vars(train.build_parser().parse_args([
        ...
        "--feature-weight", "0.05", "--mask-rate-weight", "0.25",
    ]))
    stage = SwinRecoveryStage(
        ...
        clean=tmp_path,
        source_v7=tmp_path,
        start_checkpoint=tmp_path / "v6_best.pt",
        proxy_checkpoint=tmp_path / "proxy.pt",
        train_dir=tmp_path / "train",
        controller_dir=tmp_path / "controller",
        swin_dir=tmp_path / "swin",
        saved_args=saved,
    )
    configured = vars(
        train.build_parser().parse_args(training_arguments(stage))
    )
    assert configured["resume"] is None
    assert configured["init_checkpoint"] == str(stage.start_checkpoint)
    assert configured["proxy_checkpoint"] == str(stage.proxy_checkpoint)
    assert configured["train_codec_source"] == "real"
    assert configured["rate_dual_control"] is True
    assert configured["target_bpp_ratio"] == pytest.approx(0.95)
    assert configured["feature_weight"] == 0
    assert feature_weight_map(SimpleNamespace(**configured)) == {
        30: 0.0, 35: 0.0, 40: 0.0, 45: 0.0,
    }
    assert configured["mask_rate_weight"] == 0
