...
def validate_resume_proxy(
    ...
    if old_hash is None and old_args.get("proxy_checkpoint", proxy_path) != proxy_path:
        raise ValueError(
            "resume checkpoint has no proxy SHA-256 and changed --proxy-checkpoint"
        )
    if old_hash is not None and old_args.get("proxy_checkpoint", proxy_path) != proxy_path:
        print(
            "[resume] proxy path changed but SHA-256 is identical; "
            "accepting relocated frozen proxy"
        )
    return False
