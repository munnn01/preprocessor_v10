...
class FrozenVideoAnalyzer(nn.Module):
    ...
    def forward_with_features(
        ...
        """Capture named frozen-analyzer activations without cutting input gradients."""
        if clip.ndim != 5 or clip.shape[2] != 3:
            raise ValueError(f"expected [B,T,3,H,W], got {tuple(clip.shape)}")
        ...
        if missing:
            raise ValueError(f"analyzer {self.name!r} has no feature layers {missing}")
        features: dict[str, torch.Tensor] = {}
        handles = [
            modules[name].register_forward_hook(
                lambda _module, _inputs, output, key=name: features.__setitem__(key, output)
            )
