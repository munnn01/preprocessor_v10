...
``PaperPreprocessor`` keeps the small CNN from the original implementation.
``VideoTransformerPreprocessor`` is the default architecture: it uses ViT-style
patch tokens with factorized spatial and temporal self-attention, then predicts a
bounded RGB residual.  Both modules preserve the public ``BTCHW -> BTCHW`` API.
"""
...
def build_preprocessor(
    ...
    *,
    ...
    raise ValueError(f"unknown preprocessor {kind!r}; choose 'vit' or 'cnn'")
