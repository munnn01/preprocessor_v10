...
def feature_distillation(proposed, reference, layers, weights=None, mode="cosine"):
    ...
    """
    weights = [1.0 / len(layers)] * len(layers) if weights is None else list(weights)
    if len(weights) != len(layers):
        raise ValueError("feature weights must match feature layers")
    terms = {}
    for layer in layers:
