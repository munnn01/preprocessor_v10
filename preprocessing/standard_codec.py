...
class StandardCodecProxy(nn.Module):
    """Predictive-residual virtual codec with a differentiable entropy rate.

    The first frame is predicted from mid-gray and each later frame from the
    previous source frame.  Only quantized residual latents reach the decoder;
    no unquantized encoder skip can bypass the rate bottleneck.  A monotone
    QP-conditioned affine calibration maps entropy bits to measured codec BPP.
    """

    ARCHITECTURE = "predictive_entropy_v1"
    ...
    def __init__(
        ...
        # Factorized Laplace prior in quantized-latent units.  QP conditioning
        # adjusts channel scales while a positive affine calibration accounts
        # for standard-codec headers and motion syntax.  The affine map is
        # monotone, so it cannot reverse the entropy-rate gradient.
        self.entropy_log_scale = nn.Parameter(torch.zeros(bottleneck_channels))
        self.entropy_qp_scale = nn.Linear(film_channels, bottleneck_channels)
        self.rate_calibration = nn.Linear(film_channels, 2)
        nn.init.zeros_(self.entropy_qp_scale.weight)
        nn.init.zeros_(self.entropy_qp_scale.bias)
        nn.init.zeros_(self.rate_calibration.weight)
        with torch.no_grad():
            self.rate_calibration.bias[0] = math.log(math.expm1(0.05))
            self.rate_calibration.bias[1] = math.log(math.expm1(0.005))
    ...
    @staticmethod
    def _prediction_residual(source: torch.Tensor) -> torch.Tensor:
        """I-frame signal followed by explicit previous-frame residuals."""
        first = source[:, :, :1] - 0.5
        if source.shape[2] == 1:
            return first
        return torch.cat((first, source[:, :, 1:] - source[:, :, :-1]), dim=2)

    @staticmethod
    def _laplace_cdf(value: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
        tail = (-value.abs() / scale).exp()
        return 0.5 + 0.5 * value.sign() * (1.0 - tail)

    def _entropy_bpp(
        self,
        quantized_units: torch.Tensor,
        condition: torch.Tensor,
        pixels: int,
    ) -> torch.Tensor:
        scale_delta = self.entropy_qp_scale(condition)
        scale = F.softplus(
            self.entropy_log_scale[None] + scale_delta
        )[:, :, None, None, None].clamp_min(1e-4)
        upper = self._laplace_cdf(quantized_units + 0.5, scale)
        lower = self._laplace_cdf(quantized_units - 0.5, scale)
        likelihood = (upper - lower).clamp_min(self.entropy_floor)
        bits = -torch.log2(likelihood).sum(dim=(1, 2, 3, 4))
        entropy_bpp = bits / float(pixels)
        calibration = self.rate_calibration(condition)
        gain = F.softplus(calibration[:, 0]).clamp_min(1e-6)
        overhead = F.softplus(calibration[:, 1])
    ...
    def forward(
        ...
        prediction_residual = self._prediction_residual(channels_first)
        high = self.encoder_high(self.stem(prediction_residual), condition)
        ...
        quantized_units = self._ste_round(bottleneck / q_step)
        quantized = quantized_units * q_step

        decoded_middle = self.up_middle(quantized)
        ...
        rate = self._entropy_bpp(
            quantized_units, condition, time * height * width
        )
