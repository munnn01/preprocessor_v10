...
def audit_proxy_metrics(
    ...
    *,
    ...
    max_proxy_clamp_fraction: float = 0.05,
) -> dict:
    ...
    thresholds = {
        ...
        "max_proxy_clamp_fraction": float(max_proxy_clamp_fraction),
    }
    if not qps:
    ...
    if not 0.0 <= thresholds["max_proxy_clamp_fraction"] <= 1.0:
        raise ValueError("max_proxy_clamp_fraction must be in [0, 1]")
    ...
    for qp in (int(value) for value in qps):
        names = {
            ...
            "proxy_clamp_fraction": f"qp{qp}_proxy_clamp_fraction",
        }
        ...
        if not finite:
            ...
            if values["proxy_clamp_fraction"] > thresholds["max_proxy_clamp_fraction"]:
                reasons.append("proxy_clamp_fraction_too_high")
    ...
    if len(finite_rows) == len(rows):
        ...
        mean_clamp = sum(row["proxy_clamp_fraction"] for row in rows) / len(rows)
        score = (
            mean_mape
            + 25.0 * (1.0 - mean_pair)
            + 25.0 * (1.0 - mean_down)
            + 2.0 * max(mean_real_delta, 0.0)
            + 100.0 * max(mean_clamp - thresholds["max_proxy_clamp_fraction"], 0.0)
        )
    else:
        mean_mape = mean_pair = mean_down = mean_real_delta = mean_clamp = None
        score = math.inf
    return {
        ...
        "mean_proxy_clamp_fraction": mean_clamp,
        "per_qp": rows,
    }
